...
/// @notice Abstract Uniswap v4 hook coordinating fee enforcement, protocol fee collection,
///         pluggable MEV modules, and per-pool extensions for ArtCoins-deployed pools.
/// @dev Inheriting hooks override `_setFee` and `_initializeFeeData` to specialize fee logic.
abstract contract ArtCoinsHookV2 is BaseHook, IArtCoinsHook {
    ...
    /// @notice Maximum hook-level protocol fee numerator (50% of imposed LP fee).
    uint256 public constant MAX_PROTOCOL_FEE_NUMERATOR = 500_000;
    /// @notice Hook-level protocol fee numerator. Disabled by default in
    ///         artcoins v1: the protocol's share comes from the locker reward
    ///         array (factory-injected protocol reward slot), NOT from a
    ///         separate hook skim on top of the pool fee. Setting this to
    ///         non-zero would make the trader pay strictly more than the
    ///         configured pool fee. Reserved for future hook variants.
    uint256 public protocolFeeNumerator = 0;
    /// @notice Uniswap fee denominator (1,000,000 = 100%).
    int128 public constant FEE_DENOMINATOR = 1_000_000;

    /// @notice Currently configured hook-level protocol fee, in fee-denominator
    ///         units. Always 0 in artcoins v1 (since `protocolFeeNumerator = 0`).
    ...
    /// @notice Updates the protocol fee numerator (capped at `MAX_PROTOCOL_FEE_NUMERATOR`). Factory owner only.
    /// @param newNumerator New numerator value.
    function setProtocolFeeNumerator(uint256 newNumerator) external {
        require(msg.sender == Ownable(factory).owner(), "only owner");
        require(newNumerator <= MAX_PROTOCOL_FEE_NUMERATOR, "too high");
    ...
    function _initializePool(
        ...
        if (pairedToken == address(0) || artCoin == address(0)) {
        ...
        onlyFactory
    {
        ...
        IArtCoinsMevModule(mevModule[poolKey.toId()]).initialize(poolKey, mevModuleData);
    ...
    function _lpLockerFeeClaim(PoolKey calldata poolKey) internal {
        ...
        // determine the token
        address token = artCoinIsToken0[poolKey.toId()]
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // trigger the fee claim
        IArtCoinsLpLocker(locker[poolKey.toId()]).collectRewardsWithoutUnlock(token);
    }

    /// @dev Lazy-flushes prior swap's accrued sniper-extra to the per-pool
    ///      `sniperFeeRecipient`. Mirrors `_hookFeeClaim` but uses an explicit
    ///      per-currency accumulator (rather than the hook's full claim-token
    ///      balance) so the flush never collides with the hook protocol-fee
    ///      path. Called near the top of `_beforeSwap`.
    function _sniperExtraFeeClaim(PoolKey calldata poolKey) internal {
        ...
        if (amt0 > 0) {
            ...
            poolManager.take(poolKey.currency0, recipient, amt0);
        ...
        if (amt1 > 0) {
            ...
            poolManager.take(poolKey.currency1, recipient, amt1);
    ...
    function _hookFeeClaim(PoolKey calldata poolKey) internal {
        ...
        Currency feeCurrency =
            artCoinIsToken0[poolKey.toId()] ? poolKey.currency1 : poolKey.currency0;
    ...
    function _beforeSwap(
        ...
        // prior swaps). Must run BEFORE _hookFeeClaim so the explicit
        // accumulator path doesn't collide with the hook protocol-fee claim.
        ...
        // case: specified amount paired in, unspecified amount artCoin out
        ...
        if (isExactInput && swappingForArtCoins) {
            ...
            // this also helps us match the ExactOutput swappingForArtCoins scenario
            ...
            poolManager.mint(
                address(this),
                token0IsArtCoins ? poolKey.currency1.toId() : poolKey.currency0.toId(),
                uint256(int256(fee))
            );
        }

        // case: specified amount paired out, unspecified amount artCoin in
        ...
        if (!isExactInput && !swappingForArtCoins) {
            ...
            // the ExactOutput !swappingForArtCoins scenario
            uint128 scaledProtocolFee = uint128(protocolFee) * 1e18 / (1_000_000 - protocolFee);
            int128 fee = int128(swapParams.amountSpecified * int128(scaledProtocolFee) / 1e18);
            delta = toBeforeSwapDelta(fee, 0);
        ...
        // Sniper-extra skim for exactInput swaps. Run AFTER the existing
        // protocol-fee delta logic (which is a no-op while
        // `protocolFeeNumerator == 0`) so the two paths compose without
        // collision. The MEV module signalled the extra ppm via
        // `mevModuleSetSniperFee` during `_runMevModule`. ExactOutput
        // swaps fall through to the matching `_afterSwap` branch where the
        // realized input amount is known. The slot is cleared exactly once
        // per swap, in `_afterSwap`, so the value is visible to both this
    ...
    function _afterSwap(
        ...
        // case: specified amount artCoin in, unspecified amount paired out
        ...
        if (isExactInput && !swappingForArtCoins) {
            // grab non-artCoin amount out
            int128 amountOut = token0IsArtCoins ? delta.amount1() : delta.amount0();
            // take fee from it
        ...
        // case: specified amount artCoin out, unspecified amount paired in
        ...
        if (!isExactInput && swappingForArtCoins) {
            // grab non-artCoin amount in
            int128 amountIn = token0IsArtCoins ? delta.amount1() : delta.amount0();
        ...
        // Sniper-extra skim for exactOutput swaps. Mirrors the existing
        // protocol-fee `_afterSwap` branches: read the realized input
            ...
            if (sniperPpm > 0) {
                ...
                if (!isExactInput && sniperFeeRecipient[pid] != address(0)) {
                    ...
                    if (sniperSkim > 0) {
                        ...
                        // `+=` composes safely with the existing protocol-
                        // fee branches above (which are no-ops while
                        ...
                        // Mirror the existing protocol-fee delta adjustment
                        // so `_runPoolExtension` sees post-skim amounts.
                        // The negative side is whichever currency the user
                        // paid in.
                        if (delta.amount0() < 0) {
