...
|---|---|---|---|
| `generate-abis.mjs` | Compiles a fixed list of contracts from `src/` (plus two Uniswap v4-periphery lens contracts that compile cleanly standalone) with `solc` 0.8.26 — matching `foundry.toml`'s pinned version — and writes their ABIs to `ui/src/lib/abi.generated.ts`. This is how the UI's contract ABIs stay in sync with the actual Solidity source instead of being hand-transcribed (issue #18). Deterministic: re-running with unchanged contracts produces a byte-identical file. `ui/src/lib/abi.vendor.ts` holds the few ABIs that aren't generated (no source in this repo, or a solc/remapping conflict — see that file's header for why each one). | `node script-js/generate-abis.mjs` (or `npm run gen:abi` from `script-js/`) | live |
| `check:abi` (npm script, not a standalone file) | Drift guard: regenerates `abi.generated.ts` and fails (`git diff --exit-code`) if the committed file would change — i.e. someone edited a contract without regenerating. Wire this into CI alongside the Foundry build. | `cd script-js && npm run check:abi` | live |
