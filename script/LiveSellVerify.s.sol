...
/// @title LiveSellVerify
/// @notice One small LAYER → WETH sell to verify the extension classifies
///         sells correctly. Sells 50_000 LAYER (~0.3% of CB43's ~15M holding).
contract LiveSellVerify is Script {
    ...
    function run() external {
        ...
        console2.log("Counter at start:", b0, "buys");
        console2.log("                  sells:", s0);

        vm.startBroadcast();

        PoolSwapTest swapRouter = new PoolSwapTest(IPoolManager(POOL_MANAGER));
        IERC20(LAYER).approve(address(swapRouter), 50_000 ether);

        IPoolManager.SwapParams memory params = IPoolManager.SwapParams({
            zeroForOne: true, // LAYER (currency0) → WETH (currency1)
            amountSpecified: -50_000 ether,
        ...
        console2.log("Counter at end:  ", b1, "buys");
        console2.log("                  sells:", s1);
        require(s1 - s0 == 1, "expected 1 sell recorded");
        require(b1 == b0, "buys should be unchanged");
    }
}
